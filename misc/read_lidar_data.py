...
def draw_lines_from_rho_theta(rho_values, theta_values, accumulator_values, ax):
  max_x = 200 

  if not accumulator_values:
    accumulator_values = [0] * len(rho_values)

  for i, (rho, theta, accumulator) in enumerate(zip(rho_values, theta_values, accumulator_values)):
      ...
      ax.annotate(f'Wall {i+1} (rho={rho}, theta={theta}, A={accumulator})', (mid_x, mid_y), textcoords="offset points", xytext=(10,10), ha='center')
...
def parse_file(file_path):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            ...
            elif line.startswith("** Walls accumulators: "):
                walls_accumulators = list(map(float, line.split(":")[1].strip().split(',')))
    ...
    return lidar_points, cartesian_points, walls_rho, walls_theta, walls_accumulators, corners_x, corners_y, centroid

def lidarDataPlot(cartesian_points, walls_rho, walls_theta, walls_accumulators, corners_x, corners_y, centroid):
  ...
  draw_lines_from_rho_theta(walls_rho, walls_theta, walls_accumulators, ax)
...
if __name__ == "__main__":
  ...
  #circularPlot(r"C:\Users\remye\Documents\robot-prog\misc\lidar.log")
  #cartPlot(r"C:\Users\remye\Documents\robot-prog\misc\log_cartesien.txt")
  
  lidar_points, cartesian_points, walls_rho, walls_theta, walls_accumulators, corners_x, corners_y, centroid = parse_file(r"C:\Users\remye\Documents\robot-prog\misc\lidar_data.txt")
  lidarDataPlot(cartesian_points, walls_rho, walls_theta, walls_accumulators, corners_x, corners_y, centroid)